...
import {
  PagePullOuting,
  PageResultSend,
} from '../dto/service-dto/journal-get-dto';
@ApiUseTags('journal 日志请求接口')
@Controller('/journal')
export class JournalController {
  constructor(private readonly journalExtService: JournalExtService) {}
  @ApiOperation({ title: '发送日志' })
  @ApiOkResponse({ description: '回调成功/失败', type: ResultSend })
  @UsePipes(new JournalValidationPipe())
  @Post('/upload')
  async sendJournal(
    @Body() req: JournalArrayServiceDto,
    @Ip() ip: string,
  ): Promise<ResultSend> {
    req.data.forEach(item => item.ip = ip)
  ...
  @UsePipes(new JournalValidationPipe())
  @Get('/obtain')
  async getJournalAnMany(
    @Query() query: PagePullOuting,
  ): Promise<PageResultSend> {
